...
async function getChannelActiveTheme({ accessToken, apiHost, storeHash, channelId }) {
    ...
    if (!response.ok) {
        throw new Error(
            `Could not fetch active theme details for channel ${channelId}: ${response.statusText}`,
        );
    }
    const payload = await response.json();
...
async function getThemeConfiguration({
    ...
    const reqUrl = `${apiHost}/stores/${storeHash}/v3/themes/${themeId}/configurations?uuid:in=${configurationId}`;
