 ...
 * @param callback
 */
function assemble(path, callback) {
    Fs.readdir(Path.join(path, localeDirectory), function (err, localeFiles) {
        var localesToLoad = {};
        // Ignore hidden files
        ...
        _.each(localeFiles, function (localeFile) {
            ...
            localesToLoad[localeName] = function (callback) {
                var localeFilePath = Path.join(path, localeDirectory, localeFile);
