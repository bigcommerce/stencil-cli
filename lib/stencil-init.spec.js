...
describe('StencilInit integration tests', () => {
    describe('run', () => {
        it('using cli prompts, should perform all the actions, save the result and inform the user about the successful finish', async () => {
            ...
            const saveStencilConfigSpy = jest.spyOn(stencilConfigManager, 'saveStencilConfig');
            ...
            expect(saveStencilConfigSpy).toHaveBeenCalledWith(expectedResult);
...
describe('StencilInit unit tests', () => {
    ...
    const getStencilConfigManagerStub = () => ({
        readStencilConfig: jest.fn().mockReturnValue(getStencilConfig()),
        saveStencilConfig: jest.fn(),
    });
    ...
    describe('readStencilConfig', () => {
        it("should return an empty config if the file doesn't exist", async () => {
            ...
            stencilConfigManagerStub.readStencilConfig.mockReturnValue(null);
            ...
            expect(stencilConfigManagerStub.readStencilConfig).toHaveBeenCalledTimes(1);
            expect(stencilConfigManagerStub.readStencilConfig).toHaveBeenCalledWith(true);
        ...
        it('should read the file and return parsed results if the file exists and it is valid', async () => {
            ...
            const stencilConfigManagerStub = getStencilConfigManagerStub();
            const loggerStub = getLoggerStub();
            stencilConfigManagerStub.readStencilConfig.mockReturnValue(parsedConfig);
        ...
        it('should read the file, inform the user that the file is broken and return an empty config', async () => {
            ...
            const loggerStub = getLoggerStub();
            const stencilConfigManagerStub = getStencilConfigManagerStub();
            stencilConfigManagerStub.readStencilConfig.mockRejectedValue(thrownError);
