...
describe('BundleValidator', function () {
    ...
    it ('should validate returned objects exist in templates', async () => {
        ...
        const result = await promisify(validator.validateObjects.bind(validator))(assembledTemplates);
    ...
    it ('should validate theme schema and throw errors', async () => {
        ...
        await expect(
            promisify(validator.validateTheme.bind(validator))(),
        ).rejects.toThrow('schema[0].settings[0] should have required property \'content\'');
    });
});
