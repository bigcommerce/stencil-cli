...
internals.parseAnswers = function (
    JspmAssembler,
    ThemeConfig,
    StorefrontConfig,
    dotStencilFile,
    ...
    var defaults = dotStencilFile
        ? hoek.applyToDefaults(dotStencilFile, answers)
        : answers;
...
=======
            if (err) {
                throw err;
>>>>>>> chore: wired in storefront config init
            }
            ...
            if (configuration.jspm) {
                if (
                    !Fs.existsSync(
                        Path.join(
                            themePath,
                            configuration.jspm.jspm_packages_path
                        )
                    )
                ) {
                    console.log(
    ...
    StorefrontConfig.getInstance(themePath).init();
};
...
internals.implementation = function (
    ...
    if (Fs.existsSync(dotStencilFilePath)) {
        dotStencilFile = Fs.readFileSync(dotStencilFilePath, {
            encoding: "utf-8",
        });
        try {
    ...
    questions = [
        {
            ...
            default:
                url ||
                (dotStencilFile && dotStencilFile.normalStoreUrl) ||
                undefined,
        },
        {
            type: "input",
