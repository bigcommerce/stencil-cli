import { defaultsDeep, cloneDeep, omit, inRange, isObject } from 'lodash-es';
import fs from 'fs';
...
class ThemeConfig {
    ...
    async getConfig() {
        ...
        this.globalSettings = cloneDeep(config.settings);
        ...
        config.settings = defaultsDeep(this.currentVariationSettings, config.settings || {});
        config.images = defaultsDeep(variation.images, config.images || {});
    ...
    async updateConfig(newSettings, saveToFile) {
        ...
        this.currentVariationSettings = omit(
            newSettings,
    ...
    async variationExists(variationIndex) {
        return inRange(variationIndex, 0, await this.getVariationCount());
    }
    ...
    async getSchemaTranslations() {
        ...
        return isObject(schemaTranslations) ? schemaTranslations : {};
    }
    ...
    _getVariation(config, variationIndex) {
        ...
        if (!isObject(variation.settings)) {
            variation.settings = {};
        }
        if (!isObject(variation.images)) {
            variation.images = {};
        }
        if (!isObject(variation.meta)) {
            variation.meta = {};
        }
        return variation;
    }
