...
var OBJECTS_TO_VALIDATE = ['head.scripts', 'footer.scripts'];
...
function BundleValidator(themePath, themeConfig, isPrivate) {
    ...
    this.validationTasks.push(validateDistFiles.bind(this));
}
...
function validateThemeConfiguration(callback) {
    ...
    if (!this.isPrivate && !this.themeConfig.schemaExists()) {
        console.log(
            "Warning: Your theme is missing a 'schema.json' file.".orange
        );
    }
...
function validateJspmSettings(callback) {
    ...
    if (configuration.jspm) {
        if (
            !fileExists(
                Path.join(this.themePath, configuration.jspm.jspm_packages_path)
            )
        ) {
            errorMessage = 'The path you specified for your "jspm_packages" folder does not exist.'
                .red;
            errorMessage +=
...
function validateMetaImages(callback) {
    ...
    var imagePath = Path.resolve(
        this.themePath,
        'meta',
        configuration.meta.composed_image
    );
    var imageTasks = [];
    var self = this;
    ...
    configuration.variations.forEach(
        function(variation) {
            ...
            if (!isValidImageType(imagePath)) {
                ...
                return callback(
                    new Error(
                        'The path you specified for the '.red +
                            id +
                            ' variation\'s "desktop_screenshot" does not exist.'
                                .red
                    )
                );
            } else {
            ...
            if (!isValidImageType(imagePath)) {
                ...
                return callback(
                    new Error(
                        'The path you specified for the '.red +
                            id +
                            ' variation\'s "mobile_screenshot" does not exist.'
                                .red
                    )
                );
            } else {
 ...
 */
BundleValidator.prototype.validateObjects = function(
    assembledTemplates,
    callback
) {
    Async.map(
        ...
        function(err, results) {
            ...
            results = _.difference(
                OBJECTS_TO_VALIDATE,
                _.uniq(_.flatten(results))
            );
            if (results.length !== 0) {
                return callback(
                    new Error(
                        'Missing required objects/properties: ' +
                            results.join('\n')
                    )
                );
            }
...
/**
 * Ensure dist files exists for JavaScript functionality
 * @param callback
 * @returns {*}
 */
function validateDistFiles(cb) {
    var path = Path.join(this.themePath, 'assets/dist');
    var chunkCount = 0;
    var mainExists = false;
    Fs.readdir(path, function(err, paths) {
        if (paths && paths.length) {
            for (var i = 0; i < paths.length; i++) {
                if (paths[i].includes('theme-bundle.chunk')) {
                    chunkCount++;
                } else if (paths[i].includes('theme-bundle.main')) {
                    mainExists = true;
                }
            }
            if (!mainExists || chunkCount === 0) {
                return cb(new Error('Failed to generate dist files.'));
            }
        }
    });
    cb(null, true);
