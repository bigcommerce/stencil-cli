...
<div>
    ...
    {{/each}}
            ...
            expect(results['products[0].name'].count).toBeGreaterThanOrEqual(1);
        });
...
<div>
    ...
    {{}}
        ...
        it('should handle empty template files', async () => {
            ...
            await fs.promises.writeFile(path.join(pagesDir, 'valid.html'), '<p>{{valid.variable}}</p>');
...
<div>
    ...
    <span>Static content</span>
            ...
            await fs.promises.writeFile(path.join(pagesDir, 'helpers-only.html'), helpersOnlyTemplate);
    ...
    describe('Form Component Analysis', () => {
        it('should capture form component variables in second pass', async () => {
            const testTemplatesDir = path.join(tempDir, 'templates');
            const formsDir = path.join(testTemplatesDir, 'components', 'common', 'forms');
    ...
    {{/each}}
            ...
            expect(results['form.fieldName'].paths).toEqual(
                expect.arrayContaining([
                    'components/common/forms/text.html'
                ])
            );
            expect(results['form.selectName'].paths).toEqual(
                expect.arrayContaining([
                    'components/products/options/select.html'
                ])
            );
        });
