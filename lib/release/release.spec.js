...
import StencilRelease from './release.js';
...
`;
    ...
    describe('run', () => {
        ...
        it('should throw an error that we are not on prod-main branch', async () => {
            cliOptions.branch = 'prod-main';
            const instance = createStencilReleaseInstance();
            await expect(instance.run(cliOptions)).rejects.toThrow(
                `Not on ${cliOptions.branch} branch, please checkout prod-main branch to proceed`,
            );
        });
    });
});
