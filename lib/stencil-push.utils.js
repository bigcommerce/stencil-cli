...
const { parse, validate } = require('graphql/index');
const { specifiedRules } = require('graphql/validation/specifiedRules');
...
function validateQuery(query) {
    try {
        ...
        const errors = validate(null, parsedQuery, specifiedRules);
        if (errors.length > 0) {
            console.error('Invalid GraphQL query:', errors);
            return false;
        }
        return true;
    } catch (error) {
...
const gqlRegex = /(query|mutation)\s+[\s\S]+?\{/g;
    ...
    function scanFiles(dir) {
        fs.readdirSync(dir).forEach((file) => {
            ...
            if (fs.lstatSync(filePath).isDirectory()) {
                ...
                const matches = content.match(gqlRegex);
