...
utils.getVariations = async (options) => {
    ...
    const variationId = utils.getActivatedVariation(variations, activate);
    if (!variationId) {
        return { ...options, variations };
    }

    return { ...options, variationId };
};

utils.getActivatedVariation = (variations, activate) => {
    // Activate the default variation
    if (activate === true) {
        return variations[0].uuid;
    }
