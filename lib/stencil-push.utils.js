...
utils.readStencilConfigFile = (options, callback) => {
    ...
    fs.readFile(options.dotStencilFilePath, { encoding: 'utf8' }, (err, data) => {
        ...
        return callback(null, { ...options, config: JSON.parse(data) });
    });
};
...
utils.generateBundle = (options, callback) => {
    ...
    const bundle = new Bundle(THEME_PATH, themeConfig, themeConfig.getRawConfig(), output);

    return bundle.initBundle((err, bundleZipPath) => {
        if (err) {
            // eslint-disable-next-line no-param-reassign
        ...
        return callback(
            null,
            Object.assign(options, { bundleZipPath: options.bundleZipPath || bundleZipPath }),
        );
    });
};
