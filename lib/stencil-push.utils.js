...
utils.getStoreHash = async options => {
    ...
    const storeUrlObj = Url.parse(options.config.normalStoreUrl);
...
utils.getThemes = async options => {
    const { config: { accessToken }, apiHost, storeHash } = options;
...
utils.generateBundle = (options, callback) => {
    ...
    bundle = new Bundle(THEME_PATH, themeConfig, themeConfig.getRawConfig(), output);
...
utils.uploadBundle = async options => {
    const { config: { accessToken }, apiHost, bundleZipPath, storeHash } = options;
...
utils.deleteThemesIfNecessary = async options => {
    const { config: { accessToken }, apiHost, storeHash, themeLimitReached, themeIdsToDelete } = options;
    ...
    try {
        const promises = themeIdsToDelete.map(
            themeId => themeApiClient.deleteThemeById({ accessToken, apiHost, storeHash, themeId }),
        );
...
utils.checkIfJobIsComplete = resultFilter => async options => {
    const { config: { accessToken }, apiHost, storeHash, bundleZipPath, jobId } = options;

    const result = await themeApiClient.getJob({ accessToken, apiHost, storeHash, bundleZipPath, jobId, resultFilter });
...
utils.getVariations = async options => {
    const { config: { accessToken }, apiHost, storeHash, themeId, applyTheme, activate } = options;

    if (!applyTheme) {
        return options;
    }

    const variations = await themeApiClient.getVariationsByThemeId({ accessToken, apiHost, themeId, storeHash });
    ...
    if (!activate && activate !== undefined) {
        ...
        if (!foundVariation || !foundVariation.uuid) {
            ...
            throw new Error(`Invalid theme variation provided!. Available options ${availableOptionsStr}...`);
        }
...
utils.requestToApplyVariation = async options => {
    const { config: { accessToken }, apiHost, storeHash, variationId } = options;

    if (options.applyTheme) {
        await themeApiClient.activateThemeByVariationId({ accessToken, apiHost, storeHash, variationId });
    }
