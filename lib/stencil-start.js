...
class StencilStart {
    ...
    async run(cliOptions, stencilCliVersion) {
        ...
        const initialStencilConfig = await this.readStencilConfig();
        ...
        const updatedStencilConfig = this.mergeStencilConfigData(
    ...
    async readStencilConfig() {
        const parsedConfig = await this._stencilConfigManager.readStencilConfig();

        if (!parsedConfig.normalStoreUrl || !parsedConfig.customLayouts) {
            throw new Error(
    ...
    mergeStencilConfigData(stencilConfig, storeInfoFromAPI) {
        return {
            ...stencilConfig,
