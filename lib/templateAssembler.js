...
function assemble(templates, callback) {
    getTemplatePaths(templates, function(err, templatePaths) {
        ...
        addContentBlockFiles(templatePaths, function () {
...
function getTemplateContentSync(templateFile) {
    var path = Path.join(options.templatesFolder, templateFile + options.extension);
...
function getTemplatePaths(templates, callback) {
    ...
    function resolvePartials(templateFile, callback) {
        var file = Path.join(options.templatesFolder, templateFile + options.extension);
        ...
        Fs.readFile(file, {encoding: 'utf-8'}, function(err, content) {
            ...
            match = options.partialRegex.exec(content);
            while (match !== null) {
                partialPath = match[1];
            ...
            match = options.dynamicComponentRegex.exec(content);
            ...
            Async.each(componentPaths, function(componentPath, callback) {
                Fs.readdir(Path.join(options.templatesFolder, componentPath), function(err, files) {
                    if (err) {
                        return callback(err);
                    }
...
function getContent(templatePaths, callback) {
    ...
    function getContentReducer(acc, templatePath, reduceCallback) {
        var file = Path.join(options.templatesFolder, templatePath + options.extension);
        Fs.readFile(file, {encoding: 'utf-8'}, function(err, content) {
            if (err) {
...
function addContentBlockFiles(templatePaths, done) {
    Glob('*.html', {cwd: Path.join(options.templatesFolder, 'content-blocks')}, function (er, files) {
        files.forEach(function(file) {
            var blockName = file.substr(0, file.length - 5);

            templatePaths.push(Path.join('content-blocks', blockName));
        });
