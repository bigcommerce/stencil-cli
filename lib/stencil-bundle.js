...
Bundle.prototype.generateManifest = function (taskResults, callback) {
    ...
    Readdir(this.templatesPath, ['!*.html'], (err, filePaths) => {
        ...
        const partials = filePaths.map(file => {
            return Upath.toUnix(file.replace(this.templatesPath + Path.sep, '').replace('.html', ''));
        });

        taskResults.manifest = {
            createdAt: Date.now(),
            regions: fetchRegions(taskResults.templates, partials),
            templates: partials,
        };
...
function bundleTaskRunner(callback) {
    ...
    Async.parallel(this.tasks, (err, taskResults) => {
        ...
        this.generateManifest(taskResults, () => {
            // zip theme files
...
function fetchRegions(templates, partials) {
    ...
    pages.forEach(page => {
        // Unsorted
        // let template = templates[page].join();
        // Semi Sorted
...
function buildPageTemplate(templates, page) {
    const visited = {};
    let pageTemplate = dfsBuildPageTemplate(templates, page, visited);

    // Add templates that were not visited (like DynamicComponents)
    for (let partial in templates) {
        if (!visited[partial]) {
            pageTemplate += templates[partial];
        }
    }
 ...
 * @return {String}
 */
function dfsBuildPageTemplate(templates, currentPartial, visited) {
    const partialRegex = /\{{2}>\s*([_|\-|a-zA-Z0-9\/]+)[^{]*?}{2}/g;

    visited[currentPartial] = true;

    return templates[currentPartial].replace(partialRegex, (_, partial) => {
        if (visited[partial]) {
            return '';
        }

        return dfsBuildPageTemplate(templates, partial, visited);
    });
}
