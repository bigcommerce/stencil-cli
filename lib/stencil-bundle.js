 ...
 * 
 */
Bundle.prototype.distFolderCheck = function() {
    return !Fs.existsSync(Path.join(this.themePath, '/assets/dist')) ? false : true;
}
...
function bundleTaskRunner(callback) {
    ...
    Async.parallel(this.tasks, (err, taskResults) => {
        ...
        const distExists = this.distFolderCheck();

        if (!distExists) {
            return console.error('Error: failed to generate dist files.');
        }
